...
\section{Dataset y Origen de los Datos}
Como se ha podido observar en el punto anterior de los sistemas de puntuación modernos (Tabla \ref{tab:puntuacion_laliga_fantasy}), la implementación de un sistema \textit{Fantasy} contemporáneo requiere procesar cierto número de micro-eventos por partido. Para poder usarlo y alimentar la base de datos de este Trabajo de Fin de Grado, se ha recurrido a fuentes de datos estructurados de terceros.

Después de considerar diferentes fuentes, se ha seleccionado un \textit{dataset} alojado en la plataforma \textbf{Kaggle} \cite{kaggle_dataset}, esta es un repositorio de datos con gran cantidad de bases de datos. Este conjunto de datos proporciona el nivel de detalle justo y necesario para poder usar un sistema basado en estadísticas. En el siguiente capítulo, se podrá ver como se han extraído los datos para generar el sistema de puntuación.

\subsection{Estructura y Formato de los Datos}
El \textit{dataset} seleccionado se compone de un archivo en formato  .sqlite que ha sido extraído y exportado, dividido en tablas con formato CSV, a Supabase, una plataforma muy potente y usada actualmente, de gestión de bases de datos PostgreSQL. La información extraída se puede dividir principalmente en tres bloques:

\begin{itemize}
    \item \textbf{Entidades Estáticas:} Son aquellas tablas que contienen la información inmutable, en este grupo podemos identificar las tablas de Country, Leage, Player, PlayerAttributes y Team, estas contienen información como los ids de los jugadores y de las ligas, nombres de las ligas, nombres de los países, datos de los jugadores, etc.
    \item \textbf{Entidad Match:} Esta es la tabla que contiene la información que define el contexto de un encuentro específico, incluyendo la fecha, la jornada, el árbitro, las alineaciones titulares y el resultado final.
    \item \textbf{Datos de Eventos:} Es el núcleo de los micro-eventos. Es el conjunto de columnas de la tabla Match que contiene los xmls con la información cronológica específica de los eventos de cada partido como quién marca los goles, quién da las asistencias, tiros a puerta, tiros fuera, etc. Cada columna incluye el minuto exacto, el identificador del jugador que lo realiza y, en muchos casos, sus coordenadas espaciales ($X, Y$) en el campo.
\end{itemize}

Debido a que hay un gran volumen de datos y no todos pueden ser leídos directamente, el backend diseñado parseará los datos de las columnas de eventos que contienen los xml, para posteriormente agregarlos en limpio, a tablas que pueda leer el algoritmo que se ha usado en el sistema de puntuación, para ser asignado a cada jugador. Esto se detallará a continuación.