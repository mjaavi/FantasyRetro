...
En los últimos años, el desarrollo de software ha experimentado una transformación profunda debido a la irrupción de la Inteligencia Artificial (IA) Generativa y los Modelos de Lenguaje de Gran Tamaño (LLMs, por sus siglas en inglés). Esta tecnología ha dejado de ser una mera herramienta experimental para convertirse en un componente fundamental en los flujos de trabajo de ingeniería de software modernos.

\subsection{Estado Actual de la Inteligencia Artificial Generativa}
Los modelos actuales, tales como los pertenecientes a las familias GPT de OpenAI, Claude de Anthropic y Gemini de Google, demuestran capacidades avanzadas de razonamiento lógico, traducción de lenguajes y generación de código estructurado. La capacidad de estos modelos para comprender el contexto de bases de código completas, analizar dependencias y sugerir algoritmos complejos ha propiciado su integración directa en entornos de desarrollo integrado (IDEs) a través de herramientas de asistencia como GitHub Copilot, Cursor o asistentes especializados en razonamiento de alto nivel.

\subsection{Uso de la IA en Proyectos de Ingeniería de Software}
En proyectos que implican el manejo de grandes volúmenes de datos históricos e interacciones en tiempo real, como la plataforma de simulación deportiva \textit{Retro Fantasy}, el uso de asistentes de IA se orienta a optimizar varias fases críticas:
\begin{itemize}
    ...
    \item \textbf{Diseño de consultas de base de datos y optimización}: La IA facilita la creación y verificación de scripts SQL complejos, políticas de seguridad a nivel de fila (RLS) en sistemas relacionales como PostgreSQL y el diseño de disparadores (\textit{triggers}) consistentes.
    \item \textbf{Auditoría y refactorización}: Ayuda en la identificación de fallos de lógica, redundancia de código y vulnerabilidades comunes, aplicando principios de diseño como \textit{SOLID} y \textit{Clean Architecture}.
\end{itemize}

Esta sinergia entre el desarrollador humano (que actúa como arquitecto y supervisor de la lógica del sistema) y el asistente de Inteligencia Artificial (como redactor técnico de código y depurador asíncrono) define un nuevo paradigma de programación en pareja (\textit{Pair Programming}) que redefine los tiempos de desarrollo y la fiabilidad del producto final.