...
\begin{itemize}
    ...
    \item \textbf{Género:} Simulador de Gestión Deportiva (\textit{Mánager Fantasy} asíncrono).
...
\textit{Retro Fantasy} es un mánager de fútbol virtual que permite a los usuarios revivir la época dorada de la ligas de fútbol europeo. A diferencia de los \textit{fantasy} tradicionales que funcionan en tiempo real vinculados al calendario deportivo de las ligas, Retro Fantasy se apoya en un set de datos con el que los jugadores podrán jugar una temporada desde en un año hasta en una tarde, esto le da libertad total a los jugadores y elimina uno de los problemas más importante del fantasy como son los jugadores inactivos, ya que cada liga dura el tiempo que quieran sus miembros. Al igual que en otros fantasy, el jugador asume el rol de mánager, debiendo gestionar un presupuesto limitado para fichar jugadores, crear alineaciones y competir en ligas basándose en el rendimiento real que tuvieron esos futbolistas en el pasado.

\section{Bucle de Juego y Estados}
El flujo principal de interacción del usuario está diseñado para que los usuarios decidan entre sesiones cortas, habituales en este tipo de juegos, que requiere conexiones diarias de entre 5 y 10 minutos o sesiones largas en las que los jugadores se junten para jugar la liga.
...
\begin{itemize}
    ...
    \item \textbf{Formaciones Permitidas:} El usuario tendrá que elegir entre las alineaciones disponibles para formar su once: 3-4-3, 3-5-2, 4-4-2, 4-3-3, 4-5-1 o 5-3-2. 
\end{itemize}

\section{Economía y Reglas del Mercado}
El mercado es el principal punto de interacción del juego. Toda la economía está aislada entre ligas (cada liga tiene su propia economía).

\subsection{Modelo de Inicio Híbrido}
En el unboarding de \textit{Retro Fantasy} se asigna:
\begin{itemize}
    \item \textbf{Plantilla Base:} 15 jugadores generados procedimentalmente, con un valor de mercado conjunto de entre 180 y 200 millones como máximo. Las clausulas de estos jugadores serán por defecto de un 50\% más del valor inicial.
    \item \textbf{Saldo:} 300 Millones en efectivo. Un saldo negativo impide que se pueda puntuar en la jornada siguiente.
\end{itemize}

\subsection{Tasación Inicial (Algoritmo de Precios)}
El valor de salida (precio base) de un jugador en la base de datos se calcula mediante una fórmula que tiene como objetivo equilibrar los precios entre posiciones, aplicando factores por posición:
...
\subsection{Venta Inmediata (Despido de Jugadores)}
Para agilizar la reestructuración de plantillas y dotar de liquidez inmediata a los mánagers, se establece la mecánica de \textbf{Venta Inmediata}. Cualquier mánager puede deshacerse de forma instantánea de un jugador de su propiedad bajo las siguientes reglas económicas del juego:
\begin{itemize}
    \item \textbf{Penalización por Despido:} El club recupera exactamente el \textbf{50\% del valor de mercado actual} del futbolista (redondeado hacia abajo en caso de fracciones). El 50\% restante se pierde como penalización por rescisión unilateral del contrato.
    \item \textbf{Destino del Jugador:} El jugador es liberado de manera inmediata, eliminándose de la plantilla y alineación del usuario, y pasando directamente al pozo común de agentes libres de la liga para volver a estar disponible en futuros ciclos de mercado diario.
    \item \textbf{Inmediatez y Efectivo:} El saldo líquido recuperado se añade al presupuesto disponible del mánager de forma instantánea al procesarse el despido, permitiendo reinvertir ese capital de inmediato en el mercado diario de pujas ciegas o clausulazos.
\end{itemize}

\subsection{Fluctuación Dinámica (Subidas y Bajadas)}

\begin{itemize}
    \item \textbf{Por Demanda:} Si un jugador es fichado por una puja superior a su valor, su nuevo valor de mercado absorbe el 50\% de la diferencia. Si el jugador completa el ciclo de mercado sin recibir pujas, se devalúa un 3\% diario.
    \item \textbf{Por Rendimiento:} Obtener \(\geq\) 10 puntos en una jornada incrementa su valor un 5\%. Obtener puntos negativos (por expulsiones o goles en propia) lo reduce un 5\%.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Verificación Estricta:} El motor de catálogo del sistema valida la integridad de los datos (existencia de identificadores de jugadores, coherencia de fechas y formato de eventos XML) antes de permitir su importación.
...
\section{Sistema de Puntuación (Mecánica Principal)}
...
Para emular la volatilidad de la prensa deportiva, el sistema cuenta con perfiles de cronistas virtuales (Analítico, Exigente, Pasional) que evalúan la puntuación estadística pura y, mediante algoritmos de probabilidad, otorgan \textit{Picas}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3.5cm}|c|c|c|p{4.5cm}|}
        \hline
        \textbf{Jugador y Contexto} & \textbf{LaLiga} & \textbf{Biwenger} & \textbf{Stats/Mixto} & \textbf{Conclusión del Modelo} \\
        \hline
        \textbf{K. Mbappé} \newline \textit{Valencia 0-2 R. Madrid} & 11 & 9 & \textbf{5/9} & Nota buena pero no muy alta, ya que marcó pero no destacó. \\ 
        \hline
        \textbf{Vinícius Jr.} \newline \textit{R. Madrid 4-1 R. Soc.} & 18 & 16 & \textbf{9/17} & Capta perfectamente el gran partido con doblete de goles y victoria. \\ 
        \hline
        \textbf{Lamine Yamal} \newline \textit{Girona 2-1 Barcelona} & 5 & -2 & \textbf{2/2} & El sistema no penaliza severamente por estadísticas válidas a pesar de la derrota. \\
        \hline
    \end{tabular}
...
El diseño visual tiene como objetivo desmarcarse de las aplicaciones corporativas y asépticas, acercándose a un estilo \textit{arcade}.

\subsection{Estética Retro-Futurista Neón}
\begin{itemize}
    \item \textbf{Fondo:} Espacios negativos en tonos antracita y negro absoluto (orientado a pantallas OLED de móviles).
    ...
    \item \textbf{Tipografía:} \textit{Sans-serif} modernas para las estadísticas (maximizando la legibilidad) y fuentes de corte 8-bits para títulos y marcadores.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Barra de Navegación Inferior (Bottom Bar):} Acceso rápido a las cuatro pantallas principales: Dashboard, Alineación, Mercado y Clasificación.
    \item \textbf{Patrón Bento Cards:} Las estadísticas de los futbolistas (Valor de Mercado, Histórico de Puntos, Radar de Atributos) no se muestran en listas tabulares, sino encapsuladas en tarjetas modulares con bordes redondeados (estilo \textit{Bento}), permitiendo un escaneo visual instantáneo de la jerarquía de los datos.
\end{itemize}

\section{Condiciones de Victoria y Retención}
El juego fomenta la retención del usuario al dividir el progreso histórico en \textbf{Ligas de Temporada}.
El vencedor absoluto es el mánager que acumula la mayor cantidad de puntos totales al finalizar la simulación completa de las 38 jornadas históricas. Se otorgan insignias y recompensas visuales en el perfil del mánager por hitos como "Mayor goleada", "Mánager más rico" o "Mejor racha de imbatibilidad", fomentando metas secundarias más allá de la victoria general.