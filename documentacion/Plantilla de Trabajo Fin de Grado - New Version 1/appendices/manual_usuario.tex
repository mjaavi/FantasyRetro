...
\textit{Retro Fantasy} es un simulador de gestión deportiva (\textit{Mánager Fantasy}) asíncrono. A diferencia de las plataformas tradicionales que dependen del calendario de partidos del año en curso, este juego permite a los usuarios competir en temporadas de fútbol históricas (por ejemplo, LaLiga 2011/2012 o la Premier League 2008/2009). 

Los usuarios asumen el rol de un director deportivo o mánager, compitiendo entre sí en ligas privadas. El objetivo es acumular la mayor cantidad de puntos a lo largo de las 38 jornadas de la liga, gestionando de forma eficiente un presupuesto limitado para fichar estrellas del pasado y alinear una formación competitiva.
...
\begin{itemize}
    ...
    \item \textbf{Unirse a una Liga:} Introduzca el código único de invitación (proporcionado por otro mánager administrador, con el formato \texttt{RF-XXXXXX}).
\end{itemize}

\subsection{Asignación Inicial (Modelo Híbrido)}
Al unirse a una liga, el sistema ejecuta de manera automática y transaccional el algoritmo de asignación inicial para asegurar el equilibrio de la competición:
\begin{itemize}
    \item \textbf{Plantilla Base:} Se le asignan procedimentalmente \textbf{15 futbolistas} de perfil medio-bajo pertenecientes a la liga y temporada elegidas (1 portero, 5 defensas, 6 centrocampistas y 3 delanteros). El valor de mercado conjunto de estos jugadores suma aproximadamente 180-200 millones de euros.
...
\begin{enumerate}
    \item Los futbolistas asignados en la alineación titular aparecen sobre el césped verde.
    \item Los futbolistas en el banquillo aparecen en la sección inferior derecha (\textbf{Banquillo}).
    \item Para intercambiar un jugador del banquillo por un titular, pulse sobre el slot vacío de la posición correspondiente en el campo y seleccione el sustituto deseado de la lista flotante, o pulse en el botón de intercambio rápido de su ficha.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Regla del ajuste de cláusula:} Ajustar la cláusula no tiene un coste directo en saldo, pero fijar cláusulas excesivamente bajas facilitará que los rivales le "roben" al futbolista de forma inmediata mediante un clausulazo.
\end{itemize}

\subsection{Venta Inmediata (Despido)}
Si un mánager necesita liberar espacio en su plantilla (límite máximo de 25 jugadores) u obtener dinero rápido de forma urgente para acometer un fichaje en el mercado:
\begin{enumerate}
...
\subsection{Mercado Diario (Pujas Ciegas)}
\begin{itemize}
    \item Cada 24 horas, el sistema publica una lista aleatoria de jugadores libres (agentes de la computadora) disponibles para su compra.
...
\begin{itemize}
    \item \textbf{Poner Transferible:} Un mánager puede colgar a un jugador de su plantilla en el mercado directo, estableciendo un precio mínimo de venta para indicar que está abierto a ofertas.
    \item \textbf{Enviar Oferta Directa:} Cualquier usuario de la liga puede abrir la plantilla de un rival, seleccionar un futbolista y enviarle una oferta económica directa en efectivo. El saldo de la oferta se retiene de forma virtual para garantizar la transacción.
    \item \textbf{Aceptar o Rechazar:} El propietario del jugador recibirá una notificación por email y una alerta en su panel de ofertas entrantes. Puede aceptar la oferta (el traspaso se ejecuta al instante) o rechazarla (el saldo bloqueado se devuelve al comprador).
\end{itemize}

\subsection{Clausulazos (Robo Automático)}
...
Los usuarios con roles elevados disponen de herramientas avanzadas de gestión en la pestaña de \textbf{Administración y Ajustes}.
...
\begin{itemize}
    \item \textbf{Procesar Jornada:} Introduce el número de jornada a jugar (de 1 a 38) y procesa las puntuaciones de todos los equipos.
    \item \textbf{Reiniciar Liga:} Restablece la liga a la jornada 0. Elimina todas las plantillas, devuelve los saldos a 300 millones de euros y genera una nueva asignación híbrida aleatoria de jugadores base para todos los participantes.
\end{itemize}
